...
public class PGUI extends SpigotGUIComponents {
    ...
    public void open() {
        ...
        Inventory inv = Bukkit.createInventory(null, dimension, PlayerShop.format("&3PlayerShops"));
    ...
    private void buttonsSetup(Inventory inv, Configuration messages, Configuration config) {
        ...
        try {
            for (String owner : config.getConfigurationSection("shops").getKeys(false)) {
                for (String number : config.getConfigurationSection("shops." + owner).getKeys(false)) {
                    shops++;
                    ItemStack shop = createButton(Material.valueOf(config.getString("shops." + owner + "." + number + ".shopLogo")), 1, loreButton, PlayerShop.format("&3" + config.getString("shops." + owner + "." + number + ".owner") + " " + number));
                    inv.addItem(shop);
                }
            }
